...
import type { CollaborationEvent, CollaborationEventType, DiagramCollaborationEventType } from '../types/collaboration.types'
...
function websocketUrl(): string {
  const configuredWebSocketUrl: unknown = import.meta.env.VITE_WS_URL
  if (typeof configuredWebSocketUrl !== 'string' || configuredWebSocketUrl.length === 0) {
    throw new Error('VITE_WS_URL no está configurada')
  }
  return new URL(configuredWebSocketUrl, window.location.origin).toString()
}
...
function parseEvent(message: IMessage): CollaborationEvent | null {
  try {
    ...
    if (typeof candidate.type !== 'string' || !eventTypes.has(candidate.type as CollaborationEventType)) return null
    if (typeof candidate.eventId !== 'string' || typeof candidate.timestamp !== 'string') return null
    if (typeof candidate.userId !== 'number' || typeof candidate.name !== 'string' || typeof candidate.projectId !== 'number') return null
    if (candidate.clientId !== null && candidate.clientId !== undefined && typeof candidate.clientId !== 'string') return null
    if (candidate.message !== null && candidate.message !== undefined && typeof candidate.message !== 'string') return null
    return {
...
export function connectToProject(
  ...
  const client = new Client({
    ...
    onStompError: (frame) => callbacks.onError(frame.headers.message ?? 'Error STOMP'),
    onWebSocketError: () => callbacks.onError('No se pudo conectar con colaboración'),
